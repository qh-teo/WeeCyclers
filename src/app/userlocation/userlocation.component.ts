...
export class UserlocationComponent implements OnInit {
  ...
  public setMarker(){
    ...
    myMarker.setData("Your Booking location");
  ...
  public async ngAfterViewInit() {
    ...
    var url = 'https://skippyq4.herokuapp.com/getBookingLocations';
    this.http.get(url).subscribe(data => {
      //var tmpsmartBins: any = [];
      //this.smartBins = data
      console.log(data);
      for(var i = 0; i < Object.keys(data).length; i++){       
        var dLatLng = data[i].collectionLatLong.split(",");
        
        var coladdress = data[i].collectionaddress;
        var dPoints = {lat: parseFloat(dLatLng[0]), lon: parseFloat(dLatLng[1])};
        var dist = this.distanceTo(dPoints);      
        
        
        // if(dist < 500){
          var homeMarker = new H.map.Marker(new H.geo.Point(dLatLng[0],dLatLng[1]),{icon: new H.map.Icon('/assets/rajDesign/images/home.png')});
          homeMarker.setData(coladdress);
